...
public class RangeBar extends View {
    ...
    private boolean mShowPinText = true;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        bundle.putBoolean("SHOW_PIN_TEXT", mShowPinText);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof Bundle) {
            ...
            mShowPinText = bundle.getBoolean("SHOW_PIN_TEXT");
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mIsRangeBar) {
            ...
            mLeftThumb.init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize, mCircleColor, mShowPinText);
        }
        mRightThumb = new PinView(ctx);
        mRightThumb.init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize, mCircleColor, mShowPinText);
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.mrb_RangeBar, 0, 0);
        ...
        try {
            ...
            final float tickStart = ta
                    .getFloat(R.styleable.mrb_RangeBar_mrb_tickStart, DEFAULT_TICK_START);
            final float tickEnd = ta
                    .getFloat(R.styleable.mrb_RangeBar_mrb_tickEnd, DEFAULT_TICK_END);
            final float tickInterval = ta
                    .getFloat(R.styleable.mrb_RangeBar_mrb_tickInterval, DEFAULT_TICK_INTERVAL);
            ...
            mTickHeightDP = ta
                    .getDimension(R.styleable.mrb_RangeBar_mrb_tickHeight, DEFAULT_TICK_HEIGHT_DP);
            mBarWeight = ta.getDimension(R.styleable.mrb_RangeBar_mrb_barWeight,
                    DEFAULT_BAR_WEIGHT_PX);
            mBarColor = ta.getColor(R.styleable.mrb_RangeBar_mrb_barColor, DEFAULT_BAR_COLOR);
            mTextColor = ta.getColor(R.styleable.mrb_RangeBar_mrb_textColor, DEFAULT_TEXT_COLOR);
            mPinColor = ta.getColor(R.styleable.mrb_RangeBar_mrb_pinColor, DEFAULT_PIN_COLOR);
            mActiveBarColor = mBarColor;
            mCircleSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.mrb_RangeBar_mrb_selectorSize, DEFAULT_CIRCLE_SIZE_DP),
                    getResources().getDisplayMetrics());
            mCircleColor = ta.getColor(R.styleable.mrb_RangeBar_mrb_selectorColor,
                    DEFAULT_CONNECTING_LINE_COLOR);
            mActiveCircleColor = mCircleColor;
            mTickColor = ta.getColor(R.styleable.mrb_RangeBar_mrb_tickColor, DEFAULT_TICK_COLOR);
            mActiveTickColor = mTickColor;
            mConnectingLineWeight = ta.getDimension(R.styleable.mrb_RangeBar_mrb_connectingLineWeight,
                    DEFAULT_CONNECTING_LINE_WEIGHT_PX);
            mConnectingLineColor = ta.getColor(R.styleable.mrb_RangeBar_mrb_connectingLineColor,
                    DEFAULT_CONNECTING_LINE_COLOR);
            mActiveConnectingLineColor = mConnectingLineColor;
            mExpandedPinRadius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.mrb_RangeBar_mrb_pinRadius,
            ...
            mPinPadding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.mrb_RangeBar_mrb_pinPadding,
            ...
            mBarPaddingBottom = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.mrb_RangeBar_mrb_barPaddingBottom,
                            DEFAULT_BAR_PADDING_BOTTOM_DP), getResources().getDisplayMetrics());
            mIsRangeBar = ta.getBoolean(R.styleable.mrb_RangeBar_mrb_rangeBar, true);
            mShowPinText = ta.getBoolean(R.styleable.mrb_RangeBar_mrb_pin_text, true);
