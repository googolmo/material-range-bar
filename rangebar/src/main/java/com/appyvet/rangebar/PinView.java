...
class PinView extends View {
    ...
    public void init(Context ctx, float y, float pinRadiusDP, int pinColor, int textColor,
            float circleRadius, int circleColor, boolean showText) {
        mRes = ctx.getResources();
        mPin = ctx.getResources().getDrawable(R.drawable.mrb_rotate);
    ...
    public void draw(Canvas canvas) {
        canvas.drawCircle(mX, mY, isPressed() ? mCircleRadiusPx * 1.5f : mCircleRadiusPx, mCirclePaint);
        //Draw pin if pressed
        if (mPinRadiusPx > 0 && mShowText) {
